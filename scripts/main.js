import { fetchEntries, fetchMoods, getEntries, getMoods, addNewEntry, editEntry, deleteEntry } from './dataAccess.js'
...
const convertNotebookToEntry = (entry) => {
    console.log(`entry before showing`,entry)
    notebook.innerHTML = `
    ...
    click => {
        if (click.target.id.startsWith("viewEntry--")) {
            ...
            for (const entry of entries) {
                if (entry.id === entryId) {
                    console.log(`here is the entry clicked: `, entry)
...
container.addEventListener(
     ...
     click => {
         ...
         const entryId = parseInt(click.target.id.split("--")[1])
         console.log(`here is the entry ID that will be edited --> #`,entryId)
         const entries = getEntries()
         for (const entry of entries) {
             if (entry.id === entryId) {
             console.log(entry)
...
container.addEventListener("click", click => {
    if (click.target.id.startsWith("saveEntry--")) {
        ...
        const newConcept = document.querySelector("textarea[name='newConcept']").value
        const newJournalEntry = document.querySelector("textarea[name='newJournalEntry']").value
        const newMood = document.querySelector("select[name='mood']").value
        
        const entryId = parseInt(click.target.id.split("--")[1])
        console.log(`entryId to Save: `, entryId)
        ...
        const displayNewEntry = async (id) => {
            ...
            for (const entry of entries) {
                if (entry.id === id) {
                    console.log(`entry that will re-display`,entry)
                    convertNotebookToEntry(entry)
                }
            }
        }
